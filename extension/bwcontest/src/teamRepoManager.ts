...
export async function refreshRepoState(): Promise<void> {
    ...
    if (vscode.workspace.workspaceFolders) {
        const existingOpenFolderForRepo = vscode.workspace.workspaceFolders.filter(f => f.uri.path === clonedRepoPath)[0];
...
async function cloneRepoWorker(contestId: number, teamId: number): Promise<boolean> {
    ...
    if (!fs.existsSync(clonedRepoPath)) {
        ...
        try {
            ...
            vscode.window.showErrorMessage(`BWContest: Could not create directory '${clonedRepoPath}': ${error}`);
            return false;
        }    
    ...
    try {
        const existingItemsInDir = fs.readdirSync(clonedRepoPath, {recursive: false, encoding: "utf8"});
        for (const existingItemInDir of existingItemsInDir) {
            fs.removeSync(existingItemInDir);
        }    
    }
    catch (error) {
        vscode.window.showErrorMessage(`BWContest: Failed to delete contents of Local Directory '${clonedRepoPath}': ${error}`);
        return false;
    }
...
export function getRepoPaths(contestId: number, teamId: number): { success: false } | { success: true, repoUrl: string, clonedRepoPath: string } {
