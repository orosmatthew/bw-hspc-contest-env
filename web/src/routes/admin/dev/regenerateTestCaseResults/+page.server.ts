...
export const actions = {
	regenerateMissing: async () => {
		...
		try {
			...
			return { success: true, reportJson: JSON.stringify(report), problemCount, count: submissions.length };
		}
		catch (err) {
			return { success: false, reportJson: JSON.stringify(report), problemCount, errorMessage: err?.toString() ?? "error" };
		}
	},
	regenerateAll: async () => {
		const report: string[] = [];
...
async function regenerateTestCaseResultsForSubmissions(
	...
	for (const submission of submissions) {
		...
		const analyzedOutput = analyzeSubmissionOutput(
			submission.problem,
		...
		await db.submission.update({ where: { id: submission.id }, data: { testCaseResults: analyzedOutput.databaseString } });
	}
}