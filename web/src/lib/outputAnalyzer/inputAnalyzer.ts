...
export function parseProblemInput(problem: Problem): { success: true, caseStartIndexes: number[] } | { success: false, caseStartIndexes: number[], errorMessage: string } {
    if (problem.inputSpec) {
        return parseInputCases(problem.realInput, problem.inputSpec);
    }

    return { success: false, caseStartIndexes: [], errorMessage: "Problem has no input specification." };
}

function parseInputCases(inputText: string, spec: string): { success: true, caseStartIndexes: number[] } | { success: false, caseStartIndexes: number[], errorMessage: string } {
    const result: number[] = [];
    ...
    try {
        ...
        return { success: false, caseStartIndexes: result, errorMessage: "Error thrown while parsing:" + (error?.toString() ?? '') };
    }
