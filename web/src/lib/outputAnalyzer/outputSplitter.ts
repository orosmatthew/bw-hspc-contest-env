...
export function splitJudgeOutput(outputLines: string[], totalCaseCount: number): ({ lines: string[], forcedResult: CaseResult | null } | null)[] {
    return splitOutputWithCaseLabels(outputLines, totalCaseCount);
}

export function splitTeamOutput(teamOutputLines: string[], judgeOutputLines: string[], totalCaseCount: number): ({ lines: string[], forcedResult: CaseResult | null } | null)[] {
...
function splitOutputWithCaseLabels(outputLines: string[], totalCaseCount: number): ({ lines: string[], forcedResult: CaseResult | null } | null)[] {
    ...
    for (let i = 0; i < totalCaseCount; i++) {
        if (i < outputCaseLineRanges.length) {
            ...
            const endIndexExclusive = i == totalCaseCount - 1
                ? Math.max(outputCaseLineRanges[i].endIndexExclusive, outputLines.length)
                : outputCaseLineRanges[i].endIndexExclusive;

            outputCases.push({ lines: outputLines.slice(startIndex, endIndexExclusive), forcedResult: null });
        }
        else {
    ...
    if (interruptionLines.length > 0) {
        ...
        if (lastIndexWithRecordedOutput + 1 < outputCases.length) {
            outputCases[lastIndexWithRecordedOutput + 1] = { lines: interruptionLines, forcedResult: errorIndicator!.forcedResult };
        }
        else {
...
function findErrorIndicator(lines: string[]): { lineIndex: number, forcedResult: CaseResult } | null{
...
function splitOutputWithoutCaseLabels(teamOutputLines: string[], judgeOutputLines: string[], totalCaseCount: number): ({ lines: string[], forcedResult: CaseResult | null } | null)[] {
    ...
    if (teamOutputLines.length < totalCaseCount) {
        ...
        teamOutputCases = teamOutputLines.map<({ lines: string[], forcedResult: CaseResult | null } | null)>(line => ({ lines: [line], forcedResult: null })).concat(...Array(missingLineCount).map(() => null));
    }
        ...
        teamOutputCases = teamOutputLines.map<({ lines: string[], forcedResult: CaseResult | null } | null)>(line => ({ lines: [line], forcedResult: null }));
    }
    else {
        ...
        for (let i = 0; i < totalCaseCount; i++) {
            ...
            teamOutputCases.push(teamLinesForCase.length > 0 ? { lines: teamLinesForCase, forcedResult: null } : null);
        }
    }
    ...
    if (interruptionLines.length > 0) {
        ...
        if (lastIndexWithRecordedOutput + 1 < teamOutputCases.length) {
            teamOutputCases[lastIndexWithRecordedOutput + 1] = { lines: interruptionLines, forcedResult: errorIndicator!.forcedResult };
        }
        else {
...
function caseLabelLineRanges(outputLines: string[]): { startIndex: number, endIndexExclusive: number}[] {
    ...
    return lineIndicesWithCaseLabels.map((lineIndex, i) => {
        ...
        return { startIndex, endIndexExclusive: lineIndicesWithCaseLabels[i + 1] ?? outputLines.length }
    });
}